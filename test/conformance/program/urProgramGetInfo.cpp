...
TEST_P(urProgramGetInfoTest, SuccessReferenceCount) {
  ...
  ASSERT_GET_INFO(urProgramGetInfo(program, property_name, property_size,
...
TEST_P(urProgramGetInfoTest, SuccessIL) {
  ...
  std::vector<char> property_value(property_size, static_cast<char>(0xFF));
  ASSERT_SUCCESS(urProgramGetInfo(program, property_name, property_size,
                                  property_value.data(), nullptr));
  if (property_size > 0) {
    ASSERT_SUCCESS(urProgramGetInfo(program, property_name, property_size,
...
TEST_P(urProgramGetInfoTest, NumDevicesMatchesDeviceArray) {
  ...
  ASSERT_EQ(property_value, property_value / sizeof(ur_device_handle_t));
}
...
TEST_P(urProgramGetInfoTest, NumDevicesMatchesContextNumDevices) {
  ...
  ASSERT_TRUE(property_size == 1 || property_value == property_value);
}
